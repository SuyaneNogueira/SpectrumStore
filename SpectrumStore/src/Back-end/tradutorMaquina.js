...
const MAPA_CAIXA_PRODUTO = {
  "caixa de um andar": 1,
  "caixa de dois andares": 2,
  "caixa de três andares": 3,
};
const MAPA_CAIXA_CHASSI = { preto: 1, vermelho: 2, azul: 3 };
const MAPA_CAIXA_PALETA_COR = {
  null: 0,
  vermelho: 1,
  azul: 2,
  amarelo: 3,
...
const MAPA_CAIXA_PALETA_DESENHO = {
  null: "0",
  casa: "1",
  barco: "2",
  estrela: "3",
...
function traduzirCaixaAndares(customs) {
  ...
  const traduzirAndar = (andarData) => {
    if (!andarData) {
      return DEFAULT_BLOCO_VAZIO;
    }
    return {
      cor: getOrDefault(MAPA_CAIXA_CHASSI, customs.corChassi, 1),
      lamina1: getOrDefault(
        MAPA_CAIXA_PALETA_COR,
        andarData.corPaletaEsquerda,
        0
      ),
      padrao1: getOrDefault(
        MAPA_CAIXA_PALETA_DESENHO,
        andarData.desenhoPaletaEsquerda,
        "0"
      ),
      lamina2: getOrDefault(
        MAPA_CAIXA_PALETA_COR,
        andarData.corPaletaFrontal,
        0
      ),
      padrao2: getOrDefault(
        MAPA_CAIXA_PALETA_DESENHO,
        andarData.desenhoPaletaFrontal,
        "0"
      ),
      lamina3: getOrDefault(
        MAPA_CAIXA_PALETA_COR,
        andarData.corPaletaDireita,
        0
      ),
      padrao3: getOrDefault(
        MAPA_CAIXA_PALETA_DESENHO,
        andarData.desenhoPaletaDireita,
        "0"
      ),
    };
  };
  const objetoCaixa = {
    codigoProduto: getOrDefault(
      MAPA_CAIXA_PRODUTO,
      customs.produtoEscolhido,
      1
    ),
...
const MAPA_BS_DETALHES = {
  "Com sinos": "1",
  "Com espelhos": "2",
  Espelhado: "2", // <--- Adicionei "Espelhado"
  "Com glitter": "3",
  "Com luzes": "4",
  "Com alça": "5",
  "Com velcro": "6",
...
function traduzirBrinquedosSensoriais(customs) {
  const objetoCaixa = {
    ...
    bloco1: {
      ...
      lamina3:
        customs.detalhesAdicionais && customs.detalhesAdicionais.length > 0
          ? 1
          : 0,

      // Aqui preenche padrao1, padrao2, padrao3
