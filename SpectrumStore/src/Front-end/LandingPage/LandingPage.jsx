...
// Dados para a seção de cards (Carrossel - 10 itens)
...
    // Com 10 cards e 3 por vista, temos 4 grupos (3 grupos de 3, 1 grupo de 1).
    const TOTAL_GROUPS = 4; // Ou Math.ceil(TOTAL_CARDS / CARDS_PER_VIEW);
    const [currentGroupIndex, setCurrentGroupIndex] = useState(0); 
    
    // NOVO: Estado para controlar a abertura do Modal de Cadastro
    const [isCadastroOpen, setIsCadastroOpen] = useState(false);
    
    // Lista de cards para exibir: 10 originais + 3 clones para o looping (13 no total)
...
    // Lógica para navegação circular:
    const nextGroup = () => {
        // Se estiver no último grupo de 4, volta para o primeiro (0)
        setCurrentGroupIndex((prevIndex) => (prevIndex === TOTAL_GROUPS - 1 ? 0 : prevIndex + 1));
    };

    const prevGroup = () => {
        // Se estiver no primeiro grupo, vai para o último (3)
        setCurrentGroupIndex((prevIndex) => (prevIndex === 0 ? TOTAL_GROUPS - 1 : prevIndex - 1));
    };
    
    // Calcula o valor de translação com base na largura de todos os 13 cards:
    // Ex: Slide 1: 0%. Slide 2: - (3 * 100 / 13) %
    const translateValue = currentGroupIndex * CARDS_PER_VIEW * 100 / VISUAL_CARDS_COUNT;

    return (
        <div className="landing-container">
            {/* NOVO: Renderiza o Modal de Cadastro (só se isCadastroModalOpen for true) */}
...
                    <div className="landing-cadastro-links"
                        ...
                        style={{cursor: 'pointer', marginLeft: '30px'}} // Mantém o estilo de Link, sem mudar a classe
                    >
                        Cadastre-se
                    </div>
                </div>
